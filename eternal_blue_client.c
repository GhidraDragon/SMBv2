...
/* 
 * Scans a small range around port 445 to see if anything connects 
 * (for scenarios where firewall blocks 445 but the service might be exposed).
 */
int scanAround445(const char *serverIp) {
    struct sockaddr_in addr;
    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
    if (inet_pton(AF_INET, serverIp, &addr.sin_addr) <= 0) {
        perror("inet_pton in scanAround445");
        return -1;
    }
    int foundOpen = 0;
    for (int p = 440; p <= 450; p++) {
        ...
        if (connect(sock2, (struct sockaddr*)&addr, sizeof(addr)) == 0) {
            printf("[Scanner] Possible open port %d\n", p);
            foundOpen = 1;
        }
        close(sock2);
    }
    if (!foundOpen) {
        printf("[Scanner] No open ports found in 440-450.\n");
    }
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    /* Attempt scan around 445 to see if anything is open */
    scanAround445(serverIp);
